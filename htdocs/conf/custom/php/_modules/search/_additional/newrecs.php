			...
			<div class="row h100">
				<div class="td w21 vtop pr20x pb20x">
					<ul class="left_menu">
						...
						<li><span class="aktive" onmousedown="searchNewRecs(5,'all');">Новые поступления</span></li>
...
if(isset($_POST['response']))
{
	...
	else
	{
		...
		if($N1!= 1)
		{
			...
			$globaloutput.= resPaginator($start,$length,$size,NULL,'7');
			$globaloutput.= '</p>';
		}
		$count=0;
		$textoutput="";
		...
		foreach ($response0 as $key => $value)
		{
			...
			if($res !== false)
			{
							...
							elseif($res5 !== false)
							{
								...
								if($resf !== false)
								{
									...
									else
									{
										...
										if($pos1m === false)
										{
											$see.='<span class="SEEF" onmousedown="SeeF(\''.htmlspecialchars($newtermin, ENT_QUOTES).'\',\''.$str.'\')">'.$str.'</span>';
										}
									}
								}
								if($res77 !== false)
						...
						{ 
							if($llen != "") 
							{
								if(($llen=="043")&&(isset($linkarr["043"])))
									$output.='<div class="043"><span class="url bgblue" onmousedown="showOrderWin(this,\''.$rdb.'\',\''.$theid.'\')">Заказ документа</span></div>';
				...
				$globaloutput.='<div class="table w100"><div class="row">'.$mark.'<div class="td w88x vtop">'.$imgstr.''.$slides.''.$resflstr.''.$social.'</div><div class="td vtop pr5x w100">'.$output.'</div></div></div></div>';
			}
		}
