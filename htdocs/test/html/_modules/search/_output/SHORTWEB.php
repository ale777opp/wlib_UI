...
foreach ($response0 as $key => $value)
{
	...
	if($res !== false)
	{
			...
			for($i=0; $i<$len; $i++)
			{
				if($arr[$i]!="")
				{
					...
					elseif($res3 !== false)
					{
						...
						$poslan = strpos($arr[$i], '/ebs?name=LAN');
						if($poslan === false)
						{
							$output.='<span class="URL"><a target="_blank" href="'.substr($arr[$i], $pos+1).'">Показать документ</a></span>';
						}
						else
						{
							if((isset($_POST['_auth']))||($flag45))
							{
								...
								{
									/*if($loadurl=='link')*/
										$output.='<span class="URL"><a target="_blank" href="'.substr($arr[$i], $pos+1).'&nsean='.$nsean.'">Показать документ</a></span>';
									/*else
		...
		echo '<div class="table w100"><div class="row">'.$mark.'<div class="td w88x vtop">'.$imgstr.''.$slides.''.$resflstr.''.$social.'</div><div class="td vtop p10x w100">'.$output.'</div></div></div>';
		echo '</div>';
	}
}
?>	